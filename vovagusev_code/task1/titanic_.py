...
train_data = train_data.assign(fname = train_data.Name.str.split(",").str[0]) # assign добавляет новую колонку fname(имя колонки)
train_data["title"] = pd.Series([i.split(",")[1].split(".")[0].strip() for i in train_data.Name], 
                                index=train_data.index)

test_data = test_data.assign(fname = test_data.Name.str.split(",").str[0])
test_data["title"] = pd.Series([i.split(",")[1].split(".")[0].strip() for i in test_data.Name],
                                index=test_data.index)

train_data.drop("Name", axis=1, inplace=True) # без inplace=True для удаления в train_data надо писать train_data = train_data.drop()
...
other_titles = [title
                ...
                if title not in ["Mr", "Miss", "Master", "Mme", "Mlle", "Mrs", "Ms"]] # these titles are more important than other common ones
...
#oh.fit_transform(train_data[["fname", "title"]]) к какой семье и статусу принадлежат люди 
#Male/Female 1-3 - Male 2: 1 0 0 1 0, Female 3: 0 1 0 0 1
...
interactions[['sex_class', 'Survived']].groupby(['sex_class'], 
...
train_data[['ticket_prefix', 'Survived']].groupby(['ticket_prefix'], 
                                                  as_index=False).mean().sort_values(by='Survived', ascending=False)
...
xgb_gs = GridSearchCV(xgb_model, parameters, scoring=scoring_method, cv=8, n_jobs=5)
...
catboost_gs = GridSearchCV(catBoost, test_parameters, scoring=scoring_method, cv=5, n_jobs=4)
